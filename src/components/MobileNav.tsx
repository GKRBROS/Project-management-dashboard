...
export default function MobileNav({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className={`fixed inset-0 z-50 lg:hidden ${isOpen ? '' : 'pointer-events-none'}`}>
      ...
      {/* Mobile menu */}
      <div
        className={`fixed inset-y-0 left-0 w-64 bg-[#0A0B1C] transform ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        } transition-transform duration-300 ease-in-out`}
      >
        <div className="flex flex-col h-full">
          {/* Logo */}
          <div className="flex items-center justify-center h-16 px-4 bg-[#0A0B1C]">
            <Image
              ...
              return (
                <Link
                  ...
                  className={`flex items-center px-4 py-3 text-sm font-medium rounded-md transition-colors duration-200 ${
                    isActive
                      ? 'bg-[#1C1D35] text-white'
                      : 'text-gray-300 hover:bg-[#1C1D35] hover:text-white'
                  }`}
                >
